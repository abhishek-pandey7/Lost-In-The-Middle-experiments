...
"""
================================================================================
LITM v4 — Experiment 1B: KV Retrieval (200 keys)
...
def parse_args():
    p = argparse.ArgumentParser(description="LITM v4 - Exp 1B: KV Retrieval (200 keys)")
    p.add_argument("--model", default="Qwen/Qwen2.5-1.5B-Instruct")
    p.add_argument("--n-examples", type=int, default=50)
    p.add_argument("--n-keys", type=int, default=200)
...
def main():
    args = parse_args()
    out_dir = os.path.join(args.output, "exp1b_kv200")
    os.makedirs(out_dir, exist_ok=True)

    logger.info("=" * 60)
    logger.info("LITM v4 - Experiment 1B: KV Retrieval (200 keys)")
